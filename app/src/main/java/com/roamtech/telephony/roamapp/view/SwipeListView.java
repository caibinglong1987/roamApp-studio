...
public class SwipeListView extends ListView {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			Log.e("print","motionPosition:"+motionPosition);
			if (motionPosition >= 0) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		// 编辑模式 不可点击
		if (isEditMode()||motionPosition<0) {
			setItemEnable(true);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mIsHorizontal == null) {
				if (!judgeScrollDirection(dx, dy)) {
					setItemEnable(getOpenView()==null);
					break;
				}
			}
			...
			if (mIsHorizontal) {
				...
				if (dx <0 ) {
					if(dx > -mRightViewWidth){
						mCurrentItemView.scrollTo((int) (-dx), 0);
					}else{
						mCurrentItemView.scrollTo(mRightViewWidth, 0);
					}
				}
				return true;
			} else {
	...
	// 编辑的时候点击可以关闭
	public void hiddenRightWhenEdit() {
		View openView = getOpenView();
		if (openView != null) {
			Message msg = new MoveHandler().obtainMessage();
			msg.obj = openView;
			msg.arg1 = openView.getScrollX();
			msg.arg2 = 0;
			msg.sendToTarget();
			mIsShown = false;
		}
	...
	// 刪除的时候点击可以关闭
	public void  hiddenRightWhenDelete() {
		View openView = getOpenView();
		if (openView != null) {
			openView.scrollTo(0, 0);
			mIsShown = false;
		}
	}
	// 当前被打开的View
	private View getOpenView() {
		if (mIsShown) {
			if (mPreItemView != null) {
				if (mPreItemView.getScrollX() == mRightViewWidth) {
					return mPreItemView;
				}
			}
			if (mCurrentItemView != null) {
				if (mCurrentItemView.getScrollX() == mRightViewWidth) {
