...
public class RoundImageView extends ImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawable instanceof BitmapDrawable) {
            ...
            drawColorDrawable(canvas, (ColorDrawable) drawable,radius);
        }

        if (text != null) {
            if (textPaint == null) {
                textPaint = new Paint();
                textPaint.setColor(Color.WHITE); // 设置圆环的颜色
                textPaint.setStyle(Paint.Style.FILL); // 设置空心
                //textPaint.setStrokeWidth(2); // 设置圆环的宽度
                textPaint.setAntiAlias(true); // 消除锯齿
                textPaint.setTextSize(textSize);
            }
            canvas.drawText(text,
                    (getWidth() - textPaint.measureText(text)) / 2.0f,
                    (getWidth() - (textPaint.descent() + textPaint.ascent())) / 2.0f,
                    textPaint);
        }
    }
    ...
    public void setText(String text, int textSizeSp) {
        this.text = text;
        textSize = LocalDisplay.applyDimension(TypedValue.COMPLEX_UNIT_SP, textSizeSp, getContext());
        if (textPaint != null) {
            textPaint.setTextSize(textSize);
        }
        invalidate();
    ...
    public void drawColorDrawable(Canvas canvas, ColorDrawable colorDrawable,int radius ) {
        if (mColorPaint == null) {
        ...
        mColorPaint.setColor(colorDrawable.getColor());
        canvas.drawCircle(getWidth() / 2, getHeight() / 2, radius, mColorPaint);
    ...
    private void drawCircleBorder(Canvas canvas, int radius, int color) {
        ...
        /* 设置paint的外框宽度 */
        mCirclePaint.setStrokeWidth(mBorderThickness);
        canvas.drawCircle(defaultWidth / 2, defaultHeight / 2, radius, mCirclePaint);
    }
