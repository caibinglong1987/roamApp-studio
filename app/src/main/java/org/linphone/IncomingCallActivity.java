...
public class IncomingCallActivity extends Activity implements LinphoneSliderTriggered {
	...
	protected void onResume() {
		...
		if (LinphoneManager.getInstance().getLcIfManagerNotDestroyedOrNull() != null) {
		...
		String from = CallUtil.getRealToNumber(mCall.getCallLog().getFrom().asStringUriOnly(),mCall.getCallLog().getTo().asString(), "from");
		...
		System.out.println(mCall.getCallLog().getTo().asString());
		/*final Intent intent = CallUtil.getCallIntent("10010""15657179907",
                "com.roamtech.telephony.Dialer");
