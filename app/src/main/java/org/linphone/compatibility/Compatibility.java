...
public class Compatibility {
    ...
    public static List<ContactPhone> extractContactImAddresses(String id, ContentResolver cr) {
        if (Version.sdkAboveOrEqual(Version.API09_GINGERBREAD_23)) {
            return ApiFivePlus.extractContactNumbersAndAddresses(id, cr);
        } else {
            return null;
    ...
    public static Notification createInCallNotification(Context context, String title, String msg, int iconID, Bitmap contactIcon, String contactName, PendingIntent intent) {
        ...
        if (Version.sdkAboveOrEqual(Version.API21_LOLLIPOP_50)) {
            ...
            notice = ApiSixteenPlus.createInCallNotification(context, title, msg, iconID, contactIcon, contactName, intent);
        } else if (Version.sdkAboveOrEqual(Version.API11_HONEYCOMB_30)) {
            notice = ApiElevenPlus.createInCallNotification(context, title, msg, iconID, contactIcon, contactName, intent);
        } else {
            notice = ApiFivePlus.createInCallNotification(context, title, msg, iconID, intent);
        }
