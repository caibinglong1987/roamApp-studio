...
public class ApiFivePlus {
	...
	public static List<ContactPhone> extractContactNumbersAndAddresses(String id, ContentResolver cr) {
		...
		// Phone Numbers
		Cursor c = cr.query(Phone.CONTENT_URI, new String[] { Phone.NUMBER,Phone.TYPE  }, Phone.CONTACT_ID + " = " + id, null, null);
		if (c != null) {
	        while (c.moveToNext()) {
	            String number = c.getString(c.getColumnIndex(Phone.NUMBER));
	            int type = c.getInt(c.getColumnIndex(Phone.TYPE));
	            list.add(new ContactPhone(number, type)); 
	        }
	        c.close();
		}
...
/*		String selection = new StringBuilder()
	...
	public static Cursor getGeneralContactCursor(ContentResolver cr, String select, boolean shouldGroupBy) {
		String[] projection = new String[] { Data.CONTACT_ID, Data.DISPLAY_NAME, Phone.NUMBER, "sort_key" };
		String query;

		query = Data.DISPLAY_NAME + " IS NOT NULL AND (" + select + ")";

		Cursor cursor = cr.query(Data.CONTENT_URI, projection, query, null, "sort_key COLLATE LOCALIZED ASC");//" lower(" + Data.DISPLAY_NAME + ") COLLATE UNICODE ASC");
		...
		while (cursor.moveToNext()) {
		    ...
		    if (!groupBy.contains(name)) {
		    	...
		    	int sortKey = cursor.getColumnIndex("sort_key");
		    	newRow[contactID] = cursor.getString(contactID);
		    	newRow[displayName] = cursor.getString(displayName);
		    	newRow[phoneNumber] = cursor.getString(phoneNumber);
		    	newRow[sortKey] = cursor.getString(sortKey);
	...
	public static Contact getContact(ContentResolver cr, Cursor cursor, int position) {
		try {
			if(cursor != null) {
				...
				Contact contact;
				if (input == null) {
					contact = new Contact(id, name,getContactPhoneNumber(cursor), getContactSortKey(cursor));
				} else {
					Bitmap bm = null;
					try {
					...
					contact = new Contact(id, name,getContactPhoneNumber(cursor), getContactSortKey(cursor), photo, thumbnail, bm);
				}
	...
	private static String getContactPhoneNumber(Cursor cursor) {
		return cursor.getString(cursor.getColumnIndex(Phone.NUMBER));
	}	
	private static String getContactSortKey(Cursor cursor) {
